...
WifiHttpServer::WifiHttpServer(QObject *ip_parent /*= nullptr*/)
  ...
  route(WIFI_NETWORKS,
        [this](QTcpSocket *ip_socket, const QString &i_requestStr) {
          _handleNetworkListRequest(ip_socket, i_requestStr);
        });

  route(VALIDATE_WIFI_PWD,
        [this](QTcpSocket *ip_socket, const QString &i_requestStr) {
...
void WifiHttpServer::loadWifiCredentials(const QJsonObject &i_settings) {
  ...
  if (!i_settings.contains("wifi_params") ||
      !i_settings["wifi_params"].isArray()) {
...
//-----------------------------------------------------------------------------
void WifiHttpServer::_handleNetworkListRequest(QTcpSocket *ip_socket,
...
void WifiHttpServer::_handleAuthenticationRequest(QTcpSocket *ip_socket,
  ...
  if (requestJsonObj.isEmpty()) {
    ...
    if (!requestJsonObj.contains("id") ||
        requestJsonObj["id"].toString().isEmpty()) {
      responseObj["error"] = "Missing or invalid 'id' parameter";
      status = Status::BAD_REQUEST;
    } else if (!requestJsonObj.contains("auth") ||
               requestJsonObj["auth"].toString().isEmpty()) {
      ...
      if (m_credentials.contains(networkId)) {
        if (m_credentials[networkId] == networkPass) {
          responseObj["message"] =
              QString("Accepted connection to '%1'").arg(networkId);
          status = Status::OK;
        } else {
